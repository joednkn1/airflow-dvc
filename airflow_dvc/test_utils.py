...
def execute_test_task(
    operator: BaseOperator, dag_parameter: DAG = None, *args, **kwargs
):
    post_fix = "".join(str(e) for e in choices(population=range(10), k=10))
    dag = (
        DAG(dag_id=f"test_dag{post_fix}", start_date=datetime.now())
        if dag_parameter is None
        else dag_parameter
    )
    task = operator(*args, **kwargs, dag=dag, task_id=f"test_task{post_fix}")
